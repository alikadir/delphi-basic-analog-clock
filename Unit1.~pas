...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls,  MPlayer, XPMan;
...
begin
label20.Caption:='Kafasýna göre Alarm çalýyo .. kusurabakmayýn artýk :)';
...
sann:=edsan-san;
kur.enabled:=true;
showmessage(inttostr(sat)+' saat '+inttostr(dakk)+' dakika '+inttostr(sann)+' saniye zaman kadý !!');

mediaplayer1.filename:='C:\Documents and Settings\All Users\Belgeler\Müziðim\Örnek Müzik\Beethoven 9 Nolu Senfonisi (Scherzo).wma' ;
...
label14.Caption:= timetostr(zaman)+' = zaman'+#13+#13+IntToStr(saat)+'   =   Saat'
+#13#10+#13+IntToStr(dak)+' = Dakika'+
#13#10+#13+IntToStr(san)+' = Saniye'+#13#10+#13+IntToStr(sal)+' = Salise';
...
if (akx2=a5.Left) then label6.Font.Color:=clRed else label6.Font.Color:=clNavy;
if (akx2=a10.Left) then label7.Font.Color:=clRed else label7.Font.Color:=clNavy;
if (akx2=a15.Left) then label8.Font.Color:=clRed else label8.Font.Color:=clNavy;
if (akx2=a20.Left) then label9.Font.Color:=clRed else label9.Font.Color:=clNavy;
if (akx2=a25.Left) then label10.Font.Color:=clRed else label10.Font.Color:=clNavy;
if (akx2=a30.Left) then label11.Font.Color:=clRed else label11.Font.Color:=clNavy;
if (akx2=a35.Left) then label12.Font.Color:=clRed else label12.Font.Color:=clNavy;
if (akx2=a40.Left) then label15.Font.Color:=clRed else label15.Font.Color:=clNavy;
if (akx2=a45.Left) then label16.Font.Color:=clRed else label16.Font.Color:=clNavy;
if (akx2=a50.Left) then label3.Font.Color:=clRed else label3.Font.Color:=clNavy;
if (akx2=a55.Left) then label4.Font.Color:=clRed else label4.Font.Color:=clNavy;
if (akx2=a60.Left) then label5.Font.Color:=clRed else label5.Font.Color:=clNavy;
...
{if (yelx2=a5.Left) then label6.Font.Color:=clFuchsia else label6.Font.Color:=clNavy;
if (yelx2=a10.Left) then label6.Font.Color:=clFuchsia else label7.Font.Color:=clNavy;
if (yelx2=a15.Left) then label6.Font.Color:=clFuchsia else label8.Font.Color:=clNavy;
if (yelx2=a20.Left) then label6.Font.Color:=clFuchsia else label9.Font.Color:=clNavy;
if (yelx2=a25.Left) then label6.Font.Color:=clFuchsia else label10.Font.Color:=clNavy;
if (yelx2=a30.Left) then label6.Font.Color:=clFuchsia else label11.Font.Color:=clNavy;
if (yelx2=a35.Left) then label6.Font.Color:=clFuchsia else label12.Font.Color:=clNavy;
if (yelx2=a40.Left) then label6.Font.Color:=clFuchsia else label15.Font.Color:=clNavy;
if (yelx2=a45.Left) then label6.Font.Color:=clFuchsia else label16.Font.Color:=clNavy;
if (yelx2=a50.Left) then label6.Font.Color:=clFuchsia else label3.Font.Color:=clNavy;
if (yelx2=a55.Left) then label6.Font.Color:=clFuchsia else label4.Font.Color:=clNavy;
if (yelx2=a60.Left) then label6.Font.Color:=clFuchsia else label5.Font.Color:=clNavy;
}
end;
...
if (imx2=a5.Left) then label6.Font.Size:=12 else label6.Font.Size:=8;
if (imx2=a10.Left) then label7.Font.Size:=12 else label7.Font.Size:=8;
if (imx2=a15.Left) then label8.font.Size:=12 else label8.Font.Size:=8;
if (imx2=a20.Left) then label9.Font.Size:=12 else label9.Font.Size:=8;
if (imx2=a25.Left) then label10.Font.Size:=12 else label10.Font.Size:=8;
if (imx2=a30.Left) then label11.Font.Size:=12 else label11.Font.Size:=8;
if (imx2=a35.Left) then label12.Font.Size:=12 else label12.Font.Size:=8;
if (imx2=a40.Left) then label15.Font.Size:=12 else label15.Font.Size:=8;
if (imx2=a45.Left) then label16.Font.Size:=12 else label16.Font.Size:=8;
if (imx2=a50.Left) then label3.Font.Size:=12 else label3.Font.Size:=8;
if (imx2=a55.Left) then label4.Font.Size:=12 else label4.Font.Size:=8;
if (imx2=a60.Left) then label5.Font.Size:=12 else label5.Font.Size:=8;

end;

procedure TForm1.Timer2Timer(Sender: TObject);
begin
form1.Caption:=timetostr(zaman)+'  Coded By alikadir®';
end;

procedure TForm1.Timer3Timer(Sender: TObject);
begin
form1.Caption:=' :) Coded By alikadir® (: ' ;
end;

procedure TForm1.kurTimer(Sender: TObject);
begin
if ( edsat<=saat ) then if ( eddak<=dak) then if ( edsan<=san) then
begin
mediaplayer1.open;
mediaplayer1.play;
showmessage('alarým alarým alarýýýýýýýýýýým (:');
